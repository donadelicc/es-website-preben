...
export function AlumniStartups({ startupTitle }: AlumniStartupsProps) {
  ...
  const itemsPerPage = typeof window !== 'undefined' && window.innerWidth >= 1024 ? 4 : 2;
  const totalPages = Math.ceil(startups.length / itemsPerPage);
  const visibleStartups = startups.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage);
              ...
              {visibleStartups.map((startup) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`text-gray-600 text-sm ${!isExpanded ? 'line-clamp-3' : ''} mb-8`}>
                      {startup.description && (
                        <PortableText 
